...
class Node():
    ...
    def incrementVote(self):
        ...
        if self.voteCount >= self.majority:
            print("{self.addr} becomes the leader of term {self.term}")
            self.status = LEADER
            self.startHeartBeat()
    ...
    def send_vote_req(self):
        ...
        # till everyone has voted or I am the leader
    ...
    def ask_for_vote(self, voter, term):
        ...
        while self.status == CANDIDATE and self.term == term:
            ...
            reply = self.send_message(election)
            if reply:
                choice = reply.data["choice"]
    ...
    def decide_vote(self, message):
        ...
        if self.term < message.term and (self.lastLogTerm < message.data["lastLogTerm"] or
                                             (self.lastLogTerm == message.data["lastLogTerm"]
                                              and self.lastLogIndex == message.data["lastLogIndex"])):
            self.reset_timeout()
    ...
    def reset_timeout(self):
        self.election_time = time.time() + self.nextTimeout()
    ...
    def handle_put(self, payload):
        ...
        while sum(log_confirmations) + 1 < self.majority:
            ...
            if waited > Cfg.MAX_LOG_WAIT / 1000:
                print("waited {Cfg.MAX_LOG_WAIT} ms, update rejected:")
                self.lock.release()
                return False
